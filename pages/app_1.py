...
with st.sidebar:
    ...
    st.info("""
    1. Descarga **al menos 5 archivos Excel (.xls)** del SMV (ej: 2019-2020, 2020-2021, ..., 2023-2024)
...
# ✅ Validación: al menos 5 archivos
if not archivos:
    st.warning("👆 **Por favor, sube los archivos Excel del SMV para comenzar el análisis.**")
    st.stop()
elif len(archivos) < 5:
    st.error(f"❌ **Se requieren al menos 5 archivos para realizar el análisis.** Has subido solo {len(archivos)}.")
    st.stop()
...
# ================= CREAR DATAFRAMES Y ELIMINAR PRIMER AÑO =================
...
if len(anios_comunes) > 1:
    anios_comunes = anios_comunes[1:]  # ← Elimina el primer año (el más antiguo)
if anios_comunes:
...
if not df_ratios.empty and 'Ratios' in wb.sheetnames:
    ...
    # ----------------- GENERAR GRÁFICAS CON MATPLOTLIB (EN MEMORIA) -----------------
    ...
    for idx, ratio_name in enumerate(df_ratios.index):
        ...
        if not years or not values:
            print(f"⚠️  Ratio '{ratio_name}' no tiene datos válidos. Saltando...")
            continue
        
        # Crear gráfica con matplotlib en memoria
        ...
        # ⭐️ Crear imagen para openpyxl desde el buffer
        img = XLImage(img_buffer)  # ✅ Pasar el buffer directamente al constructor
        img.width = 400
        img.height = 250
        ...
        print(f"✅ Gráfico {idx + 1}: {ratio_name} insertado en celda {cell_pos}")
